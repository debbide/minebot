...
// @version      3.1
// @description  全功能版：24h时间监控 + 拟人化点击 + 状态机持久化 + 后台防休眠 (适配 Vernieuw Nu)
// @author       Gemini
// @match        https://hyper-hub.nl/server/manage?id=*
// @grant        GM_addStyle
// @grant        GM_setValue
// @grant        GM_getValue
// ==/UserScript==
...
(function() {
    ...
    const CONFIG = {
        ...
        checkInterval: 5000,     // 扫描频率 5秒
        refreshCooldown: 300000, // 24h内若无按钮，每5分钟刷新页面一次
        btnRegex: /Vernieuw\s*Nu/i // 适配图片中的 "Vernieuw Nu"
    };
    ...
    panel.innerHTML = `
        <div id="vt-header">
            <span><span class="vt-tag">V3.1 FINAL</span> 自动续期面板</span>
    ...
    async function mainEngine() {
        if (data.isPaused || data.status === 'renewing') {
            document.getElementById('vt-running-indicator').innerText = data.isPaused ? "○ PAUSED" : "⚙️ PROCESSING";
        ...
        if (btn) {
            data.status = 'renewing'; // 锁定状态，防止重复触发
            displayStatus.innerText = "发现 Vernieuw Nu，准备点击";
            displayStatus.style.color = "#2ecc71";

            addLog("🎯 探测到续期按钮，拟人化等待中...", "info");
            await sleep(3000, 6000); // 随机等待3-6秒

            addLog("🚀 执行续期操作！", "success");
            btn.click();

            // 点击后锁定 UI，5秒后尝试刷新页面确认结果
            setTimeout(() => { 
                data.status = 'idle';
                save();
                location.reload(); 
        ...
        if (timeData) {
            ...
            displayTime.innerText = "未检测到时间文本";
            displayStatus.innerText = "正在扫描页面结构...";
        }
    }
    ...
    // --- 8. 启动 ---
    renderLogs();
    addLog("✅ Valtoria v3.1 (Dutch) 加载成功", "info");
